...
class voMembershipsFromComanage(object):
  ...
  def __init__(self, voName, userid, status, created):
    self.status = status
...
#     return data
  ...
  def getAllVoMembershipsByStatus(self, status):
    ...
    SELECT cm_cous.name, identifier, cm_co_person_roles.status,cm_co_person_roles.created
    ...
    data = []
    for row in result:
      vosData = voMembershipsFromComanage(row[0], row[1], row[2], row[3])
      data.append(vosData)
    return data
